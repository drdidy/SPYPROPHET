...
def test_live_endpoint_populates_trigger_target(monkeypatch):
    deps.get_cache().clear()
    monkeypatch.setattr(
        live_route,
        "fetch_spy_spot_snapshot",
    ...
    assert body["trigger"]["value"] == 720.07
    assert body["target"] is not None
    assert body["target"]["line_code"] == "LD"
    assert body["target"]["value"] == 716.67
    assert "720.07" in body["decision_label"]
